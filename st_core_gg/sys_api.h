...
#define NVIC_CAN_PRIO     4
#define NVIC_CAN_EX_PRIO  4

#define NVIC_USB_FS_PRIO  8

#define NVIC_EXINT_PRIO   5     //KSZ8851

#define NVIC_UART0_PRIO   5
// UART1~4, 可能需要使用timer2提供的定时功能(485使能)，所以中断优先级必须与timer1相同
#define NVIC_UART6_PRIO   5
#define NVIC_UART2_PRIO   5
#define NVIC_UART3_PRIO   5
#define NVIC_UART4_PRIO   5
...
void memdump_net(unsigned char *buf, int len);
extern "C" int my_sprintf(char *buf, int buf_len, const char *fmt, ...);
#define mi_printf LOG_PRINT


#if 1 //def MI_ASSERT_EN
#define mi_assert(n)  { if((n)==0) {  while(1){ sleep(100); mi_printf("assert failed %s line %d (%d)\r\n", __FILE__, __LINE__,(n));}}}
#else
#define mi_assert(n)
#endif // _DEBUG_
